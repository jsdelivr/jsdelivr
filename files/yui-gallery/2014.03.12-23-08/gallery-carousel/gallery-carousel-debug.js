...
Y.Carousel = Y.extend(Carousel, Y.Widget, {
    ...
    startAutoPlay: function () {
        ...
        if (JS.isUndefined(self._autoPlayTimer)) {
            ...
            Y.log("Starting auto scroll", "info", Carousel.NAME);
    ...
    stopAutoPlay: function () {
        ...
        if (!JS.isUndefined(self._autoPlayTimer)) {
            Y.log("Stopping auto scroll", "info", Carousel.NAME);
    ...
    renderUI: function () {
        ...
        if (self.get("numItems") < 1) {
            Y.log("No items to render", "warn", Carousel.NAME);
        ...
        Y.log("boundingBox: " + self.get("boundingBox"), "info", Carousel.NAME);
        Y.log("contentBox: " + self.get("contentBox"), "info", Carousel.NAME);
    },
    ...
    _parseItems: function () {
        ...
        items.each(function (item, i) {
            if (!self.addItem(item)) {
                Y.log("Unable to add parsed item - " + item, "warn",
                      Carousel.NAME);
            }
        }, self);
    },
    ...
    _setVertical: function (attrVal, attrName) {
        Y.log("_setVertical called with " + attrVal + " and " + attrName,
              "info", Carousel.NAME);
    ...
    _uiSetHeight: function (val) {
        ...
        if (val) {
            Y.log("_uiSetHeight called with " + val, "info", Carousel.NAME);
    ...
    _uiSetHeightCB: function (val) {
        ...
        if (val) {
            Y.log("_uiSetHeightCB called with " + val, "info", Carousel.NAME);
    ...
    _uiSetVertical: function (val) {
        ...
        Y.log("_uiSetVertical called with " + val, "info", Carousel.NAME);
        if (val) {
    ...
    _uiSetWidth: function (val) {
        ...
        if (val) {
            Y.log("_uiSetWidth called with " + val, "info", Carousel.NAME);
    ...
    _uiSetWidthCB: function (val) {
        ...
        if (val) {
            Y.log("_uiSetWidthCB called with " + val, "info", Carousel.NAME);
    ...
    _validateNumVisible: function (attrVal, attrName) {
        Y.log("_validateNumVisible called with " + attrVal + " and " + attrName,
              "info", Carousel.NAME);
        return attrVal > 0;
    },
    ...
    _validateRevealAmount: function (attrVal, attrName) {
        Y.log("_validateRevealAmount called with " + attrVal + ", " + attrName,
              "info", Carousel.NAME);
