...
Y.extend(tmpPanel, Y.Overlay, {
	...
	initializer : function () {
		Y.log('initalizer','info','tmpPanel');
	...
	syncUI : function () {
		Y.log('syncUI','info','tmpPanel');
	},

	//destructor
	destructor : function() {
		Y.log('destructor','info','tmpPanel');
	},


	//close button methods
	_closeButton_create : function () {
		Y.log('_closeButton_create','info','tmpPanel');
	...
	_underlay_create : function () {
		Y.log('_underlay_create','info','tmpPanel');
	},
	_underlay_destroy : function () {
		Y.log('_underlay_destroy','info','tmpPanel');
	},
	_underlay_changed : function () {
		Y.log('_underlay_changed: '+(this.get('underlay')?'true':'false'),'info','tmpPanel');
	},


	//mask methods
	_mask_create : function () {
		Y.log('_mask_create','info','tmpPanel');
	...
	_mask_destroy : function () {
		Y.log('_mask_destroy','info','tmpPanel');
		if(this._hasModality){
	...
	_mask_changed : function () {
		Y.log('_mask_change','info','tmpPanel');
		if(this.get('modal') && this.get('visible')){
			this._mask_create();
	...
	_mask_size : function () {
		Y.log('_mask_size','info','tmpPanel');
		var oMask = Y.one('#'+this.get('boundingBox')._node.id+'_mask'),
	...
	_position_update : function () {
		if(this.get('visible')){//if it's centered and visible
			Y.log('_position_update','info','tmpPanel');
