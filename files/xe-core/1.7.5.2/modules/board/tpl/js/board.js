...
function completeDocumentInserted(ret_obj){var error=ret_obj.error;var message=ret_obj.message;var mid=ret_obj.mid;var document_srl=ret_obj.document_srl;var category_srl=ret_obj.category_srl;var url;if(!document_srl){url=current_url.setQuery("mid",mid).setQuery("act","")}else{url=current_url.setQuery("mid",mid).setQuery("document_srl",document_srl).setQuery("act","")}if(category_srl){url=url.setQuery("category",category_srl)}location.href=url}
/* delete the document !*/
...
/* document search !*/
function completeSearch(ret_obj,response_tags,params,fo_obj){fo_obj.submit()}function completeVote(ret_obj){var error=ret_obj.error;var message=ret_obj.message;alert(message);location.href=location.href}function completeReload(ret_obj){var error=ret_obj.error;var message=ret_obj.message;location.href=location.href}
