...
FormKeeperAttributable.limpiar = function (cb, ask) {
  ...
  if (typeof ask !== 'string' && (typeof ask !== 'boolean' || ask === undefined || ask)) {
    cnfrm = window.confirm('¿Desea eliminar toda la información guardada por FormKeeperAttributable?')
  } else {
...
FormKeeperAttributable.guardados = function () {
  let cuenta = 0

  const bjtFormKeeper = window.localStorage.getItem('FormKeeperAttributable') !== null ? JSON.parse(this.decode(window.localStorage.getItem('FormKeeperAttributable'))) : null

  if (bjtFormKeeper[fk.identificador] !== null) {
    const keyts = Object.keys(bjtFormKeeper[fk.identificador])
    for (let i = 0; i < keyts.length; i++) {
      if (bjtFormKeeper[fk.identificador][keyts[i]] !== undefined && bjtFormKeeper[fk.identificador][keyts[i]] !== null) cuenta++
    }
  } else {
    cuenta = 0
  }
...
const fk = {
  ...
  limpiarCallback: () => {
    console.log('FormKeeperAttributable limpiado con éxito.')
  }
}
...
if (fk.domEls.length === 0) console.warn('No hay ningún elemento con el atributo "FormKeeper", tan solo añada dicho atributo al elemento o elementos con los que desea trabajar. Ejemplo: <input id="miInput" name="miInput" FormKeeper>. O bien, puede leer la documentación en https://github.com/EdGraVill/FormKeeper/tree/master/Attributable')
...
for (let i = 0; i < fk.domEls.length; i++) {
  ...
  if (thisDomEl.tagName === 'INPUT' || thisDomEl.tagName === 'TEXTAREA') {
    if (thisDomEl.type === 'checkbox') {
      thisDomEl.setAttribute('onchange', `FormKeeperAttributable.saveValue(${i}, this.checked, '${fk.identificador}')`)
    } else if (thisDomEl.type === 'range' || thisDomEl.type === 'color') {
      thisDomEl.setAttribute('onchange', `FormKeeperAttributable.saveValue(${i}, this.value, '${fk.identificador}')`)
      ...
      thisDomEl.setAttribute('onkeyup', `FormKeeperAttributable.saveValue(${i}, this.value, '${fk.identificador}')`)
    } else {
      thisDomEl.setAttribute('onkeyup', `FormKeeperAttributable.saveValue(${i}, this.value, '${fk.identificador}')`)
      thisDomEl.setAttribute('onchange', `FormKeeperAttributable.saveValue(${i}, this.value, '${fk.identificador}')`)
    }
    ...
    for (let j = 0; j < thisDomEl.length; j++) {
      thisDomEl[j].setAttribute('onchange', `FormKeeperAttributable.saveRadio(${i}, this.checked, '${fk.identificador}', [${thisDomEl.length},${j}])`)
    }
  } else {
    thisDomEl.setAttribute('onchange', `FormKeeperAttributable.saveValue(${i}, this.value, '${fk.identificador}')`)
  }
