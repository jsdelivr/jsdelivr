        ...
        this.context.lineWidth = this.properties['chart.linewidth'];
            ...
            for (var i=0; i<this.data.length; ++i) {
                ...
                context.strokeStyle = this.Get('chart.strokestyle');
                    ...
                    var x      = AA(this, this.gutterLeft + hmargin + (((this.graphwidth / (this.data.length + (this.Get('chart.total') ? 1 : 0))) * i) * this.Get('chart.multiplier.x')));
                    var y      = AA(this, this.gutterTop + this.grapharea - (i == 0 ? ((this.data[0] / this.max) * this.grapharea) : (this.data[i] > 0 ? ((runningTotal + this.data[i]) / this.max) * this.grapharea : (runningTotal / this.max) * this.grapharea)));
