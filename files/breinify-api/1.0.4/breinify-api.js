...
!function (scope) {
    ...
    /**
     * Check if the specified instance (name) is available in the specified (scope).
     *
    ...
    /**
     * Method to validate if the specified dependencies (instances) are available within the scope.
     *
     * @param scope {object} the scope to check the dependencies in
     * @param names {Array<string>} the names to check in the scope
     * @returns {boolean} if the check was successful
     */
    misc.checkMultiple = function (scope, names) {

        for (var i = 0; i < names.length; i++) {
            if (!misc.check(scope, names[i])) {
                return false;
            }
        }
    ...
    misc.isInteger = function (str) {
        if (typeof str === 'number') {
            str = '' + str;
        }

        var n = ~~Number(str);
        return String(n) === str ? n : false;
    };

    misc.isPositiveInteger = function (str, includeZero) {
        includeZero = typeof includeZero === 'boolean' ? includeZero : false;

        var n = misc.isInteger(str);
        if (n === false) {
            return false;
        } else {
            return n > 0 ? n : (includeZero && n === 0 ? 0 : false);
        }
    };

    misc.endsWith = function (str, suffix) {
        if (str === null || suffix === null) {
            return false;
        } else if (typeof str !== 'string') {
            throw new Error('The value to be validated must be a string.');
        }

        return str.indexOf(suffix, str.length - suffix.length) !== -1;
    };

    misc.inherits = function (SubType, SuperType) {
        var SubTypeProto = Object.create(SuperType.prototype);

        for (var propName in SubType.prototype) {
            if (hasOwnProperty.call(SubType.prototype, propName)) {
                SubTypeProto[propName] = SubType.prototype[propName];
            }
        }

        SubType.prototype = SubTypeProto;
        SubType.prototype.constructor = SubType;
    };

    misc.is = function (value, def) {
    ...
    misc.str = function (value, def, allowEmpty) {
        var typeOf = typeof value;
...
!function (scope) {
    ...
    /**
     * Constructor of the IntervalTask.
     *
     * Notes:
     *  - the <i>runOnStart</i> is ignored if the <i>firstDelay</i> is set to <i>0</i>
     *
     * @param {int} interval the interval to run the task in milliseconds
     * @param {function} task the task to be performed
     * @param {int} [firstDelay=interval]   the first delay in milliseconds, i.e., the time to wait until the first execution
     * @param {boolean} [runOnStart=false] true if the task should be executed once, when the start method is executed; otherwise false (default)
     * @constructor
     */
    var IntervalTask = function (interval, task, firstDelay, runOnStart) {

        if (typeof task !== 'function') {
            throw new Error('The specified task must be a function.');
        ...
        this.firstDelay = typeof firstDelay === 'number' ? parseInt(firstDelay) : this.interval;
        this.runOnStart = typeof runOnStart === 'boolean' ? runOnStart && this.firstDelay != 0 : false;
        this.task = task;
        this.handler = null;

        if (this.interval <= 0) {
            throw new Error('The interval must be a positive integer');
        } else if (this.firstDelay < 0) {
            throw new Error('The first delay must be a positive integer or 0');
        }
    };

    /**
     * Method used to calculate a first delay, so that the first task starts with the next minute.
     *
     * @param {int} [delay=0] the delay in milliseconds to be added after the next minute
     * @returns {int} the resulting first delay to be used when creating the task
     */
    IntervalTask.startOnNextMinute = function (delay) {
        delay = typeof delay === 'undefined' || delay === null ? 0 : parseInt(delay);
        return (60 - new Date().getSeconds()) * 1000 + delay;
    };

    IntervalTask.prototype = {
        constructor: IntervalTask,
        ...
        start: function () {
            ...
            if (this.firstDelay === this.interval) {
                doInterval();
            } else if (this.firstDelay === 0) {
                instance.task();
                doInterval();
            ...
            // check if we should start it on start directly
            if (this.runOnStart) {
                instance.task();
            }
        },
    ...
    misc.export(scope, 'IntervalTask', IntervalTask);
}(window);;
!function (scope) {
    var resizeListenerDataName = 'resize-listener';
    ...
    var layouts = {
        ...
        vertical: function ($, $el) {
            ...
            $el.children().each(function () {
                ...
                if (misc.endsWith(width, '%')) {
                    width = width.slice(0, -1);
                    isPercentage = true;
                } else if (misc.endsWith(width, 'px')) {
                    width = width.slice(0, -2);
                } else if (width === '-1') {
                    width = $child.width();
                }

                // let's see if width matches
                if ((width = misc.isPositiveInteger(width)) === false) {
                    throw new Error('The specified width "' + $child.attr('data-width') + '" is invalid.');
                } else if (isPercentage) {
                    takenPercentage += width;
                    layoutedEls.push({
                        $el: $child,
                        relative: width,
                        width: 0
            ...
            // the percentage once have to be calculated
            var availableWidth = totalWidth - takenWidth;
            if (availableWidth > 0 && takenPercentage > 0) {
                var offset = 0.0;

                $.each(layoutedEls, function (idx, layoutedEl) {
                    if (layoutedEl.relative !== false) {
                        var exactWidth = (layoutedEl.relative / 100) * availableWidth;
                        var width = Math.floor(exactWidth);

                        offset += exactWidth - width;
                        if (offset >= 1.0) {
                            width += 1;
                            offset -= 1;
                        }

                        layoutedEl.width = width;
                        layoutedEl.relative = false;
                    }
                });
            }
            ...
            $.each(layoutedEls, function (idx, layoutedEl) {
                ...
                var w = Math.max(0, layoutedEl.width - ($layoutedEl.outerWidth(true) - $layoutedEl.innerWidth()));
                var h = Math.max(0, height - ($layoutedEl.outerHeight(true) - $layoutedEl.innerHeight()));
                $layoutedEl.width(w);
                $layoutedEl.height(h);

                currentPos += layoutedEl.width;
            });
        }
    };

    // helper method to see if the data-resize is set
    var isResize = function ($el) {
        var resize = $el.attr('data-resize');
        return typeof resize === 'string' && resize.toLowerCase() === 'true';
    };
    ...
    /**
     * Used to layout the children of the specified $el.
     *
     * @param {function} $ the jQuery implementation
     * @param {object} $el the selected jQuery element, for which the children should be aligned
     */
    layouter.layout = function ($, $el) {
        if ($el.length !== 1) {
            throw new Error('The element must select exactly one element.');
        }

        // get the layouter to be used for the element
        var layoutName = $el.attr('data-layout');
        if (typeof layoutName !== 'string') {
            return;
        }

        // get the layout to be used
        var layout = layouts[layoutName.toLowerCase()];
        if (typeof layout === 'undefined') {
            throw new Error('Invalid layout "' + layoutName + '" specified.');
        }

        // check if resizing is defined for this element and if a resizer is known
        if (isResize($el)) {
            layouter.bindToResizer($, $el, function () {
                layout($, $el);
            });

            // check the children for resizing
            $el.children().each(function () {
    ...
    layouter.layoutAll = function ($, selector) {
    ...
    layouter.bindToResizer = function ($, $el, callback) {
        if (typeof callback !== 'function') {
            throw new Error('The callback must be a function.');
        }

        var $resizeListener = layouter.resolveResizer($, $el);

        // unbind any current resize event with the specified name
        layouter.unbindToResizer($, $resizeListener, 'resize');

        var resizer = function () {
            callback();
            $el.triggerHandler('resize');
        };

        $resizeListener.on('resize', resizer);
        $.data($el.get(0), resizeListenerDataName, resizer);

        resizer();
    };

    layouter.unbindToResizer = function ($, $el, $resizeListener) {
        if (typeof $resizeListener !== 'undefined' && $resizeListener !== null &&
            $resizeListener instanceof $ && $resizeListener.length === 1) {

            var el = $el.get(0);
            var currentResizer = $.data(el, resizeListenerDataName);
            if (typeof currentResizer !== 'undefined' && currentResizer !== null) {
                $resizeListener.removeData(el, resizeListenerDataName);
                $resizeListener.off('resize', currentResizer);
            }
        }
    };

    layouter.resolveResizer = function ($, $el) {
        var $resizer = $el.parent().closest('[data-resize=true]');
        return $resizer.length === 1 ? $resizer : $(window);
    };

    //noinspection JSUnresolvedVariable
    misc.export(scope, 'layouter', layouter);
}(window);;
!function (scope) {
    ...
    Observable.prototype = {
        ...
        off: function (event, removeObserver) {
            var observers = this.observers[event];
